...
/*float Utility::StringToFloat(const char * str, bool *outDidError){
    static_assert(sizeof(float) >= sizeof(float), "This function is used to parse strings to variables of size u32. Thus unsigned long must be at least as big.");
    char *endPtr = nullptr;
    str = IgnoreZeroPrefix(str);

    const float retVal = strtof(str, &endPtr);
    if (endPtr == str || (*endPtr != '\0' && *endPtr != '\r' && *endPtr != '\n'))
    {
        if (outDidError == nullptr) { SIMEXCEPTION(NotANumberStringException); }
        else { *outDidError = true; }
...
template<typename T>
static T StringToF(const char *str, bool *outDidError)
{
    const float retVal = Utility::StringToFloat(str, outDidError);
    if (retVal > std::numeric_limits<T>::max())
    {
        //In case no pointer to didError was given, we should throw an exception
        if (outDidError == nullptr) { SIMEXCEPTION(NumberStringNotInRangeException); }
...
float Utility::StringToFF(const char * str, bool *outDidError)
{
    return StringToF<float>(str, outDidError);
}//add */
