...
ErrorType FruityHal::BleStackInit()
{
    ...
    ble_cfg.conn_cfg.params.gap_conn_cfg.conn_count   = GS->config.totalInConnections + GS->config.totalOutConnections;  //orginal GS->config.totalInConnections + GS->config.totalOutConnections;
    ble_cfg.conn_cfg.params.gap_conn_cfg.event_length = 10; //4 units = 5ms (1.25ms steps) this is the time used to handle one connection //orginal 4 修改conneciotn event length
    ...
    trace("%u",ble_cfg.conn_cfg.params.gap_conn_cfg.conn_count);//add

    // Configure the connection roles.
    CheckedMemset(&ble_cfg, 0, sizeof(ble_cfg));
    ble_cfg.gap_cfg.role_count_cfg.periph_role_count  = GS->config.totalInConnections; //orginal GS->config.totalInConnections
    ble_cfg.gap_cfg.role_count_cfg.central_role_count = GS->config.totalOutConnections; //orginal GS->config.totalOutConnections
...
void FruityHal::DispatchBleEvents(void const * eventVirtualPointer)
{
    ...
    case BLE_GATTS_EVT_EXCHANGE_MTU_REQUEST:
    {
        ...
        logt("NODE", "Reply MTU Exchange (%u) on conn %u with %u", err, bleEvent.evt.gatts_evt.conn_handle, effectiveMtu);
    ...
    case BLE_GATTC_EVT_EXCHANGE_MTU_RSP:
    {
        ...
        logt("NODE", "MTU for hnd %u updated to %u", bleEvent.evt.gattc_evt.conn_handle, effectiveMtu);
    ...
    case BLE_GAP_EVT_PHY_UPDATE_REQUEST:
        {
            ...
            ble_gap_phys_t phy;
            phy.rx_phys = BLE_GAP_PHY_1MBPS; //orginal BLE_GAP_PHY_1MBPS
            phy.tx_phys = BLE_GAP_PHY_1MBPS; //orginal BLE_GAP_PHY_1MBPS
...
ErrorType FruityHal::BleGapScanStart(BleGapScanParams const &scanParams)
{
    ...
    scan_params.scan_phys = BLE_GAP_PHY_1MBPS; //orginal BLE_GAP_PHY_1MBPS
...
ErrorType FruityHal::BleGapConnect(FruityHal::BleGapAddr const &peerAddress, BleGapScanParams const &scanParams, BleGapConnParams const &connectionParams)
{
    ...
    p_scan_params.scan_phys = BLE_GAP_PHY_1MBPS; //orginal BLE_GAP_PHY_1MBPS
