...
class DebugModule: public Module
{
    private:
        ...
        static constexpr int SIZEOF_DEBUG_MODULE_START_COUNTER_MESSAGE = 8; //orginal static constexpr int SIZEOF_DEBUG_MODULE_START_COUNTER_MESSAGE = 8;
        typedef struct
        {
            NodeId counterDestinationId;
            u16 packetsPer10Sec; // u16 packetsPer10Sec;
            u32 maxCount;
            //u16 payload; //add
        ...
        static constexpr int SIZEOF_DEBUG_MODULE_COUNTER_MESSAGE = 8; //orginal static constexpr int SIZEOF_DEBUG_MODULE_COUNTER_MESSAGE = 8;
        typedef struct
        {
            u32 counter;
            u32 time = FruityHal::GetRtcMs();//add 這裡新增時間
            //u16 payload; //add
        } DebugModuleCounterMessage;
        STATIC_ASSERT_SIZE(DebugModuleCounterMessage, 8);//orginal 4
