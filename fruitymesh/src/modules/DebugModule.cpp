...
u16 sizepacket = 8; //orginal 20
u32 idtimedelay[7] = {0,0,0,0,0,0,0};
u32 idcounter[7] = {0,0,0,0,0,0,0};
...
//WARNING: TIME_SYNC_TEST_CODE is only for testing and will use a delay of 2 seconds!
    ...
    if (syncTest)
    {
        if (seconds % 10 == 0) {
            ...
            GS->ledGreen.On();
            FruityHal::DelayMs(200); //orginal 2000
        }
    }
#endif
    ...
    if(currentCounter <= counterMaxCount){
        if (counterMessagesPer10Sec != 0)
        {
            ...
            u32 ms = time%1000;
            u32 tempsec = time/1000;
            u32 sec = tempsec%60;
            u32 tempmin = tempsec/60;
            u32 min = tempmin%60;
            u32 temphour = tempmin/60;
            u32 hour = temphour%24;

            if (numPacketsToSend > 0) { 
                //trace("Queuing %u counter packets at time %u\n" EOL, numPacketsToSend, GS->appTimerDs);//logt
                trace("Queuing %u counter packets at time %02dh %02dm %02ds %03d number %u" EOL , numPacketsToSend, hour,min,sec,ms,currentCounter);//logt
            }
            ...
            for (u32 i = 0; i < numPacketsToSend; i++)
            {
                ...
                //data.time = FruityHal::GetRtcMs();//add
                ErrorTypeUnchecked err = SendModuleActionMessage(
                    ...
                    0,
                    (u8*)&data, // (u8*)&data
                    sizepacket,//   sizepacket SIZEOF_DEBUG_MODULE_COUNTER_MESSAGE
                    false);
    ...
    if(lastFloodPacketMs && lastFloodPacketMs + 3000 < FruityHal::GetRtcMs()){
        ...
        trace("Counted %u flood payload bytes in %u ms = %u byte/s" EOL, autoFloodSum, passedTimeMs, throughputInBytesPerSecond);// find
    ...
    if(floodMode != FloodMode::OFF){
        ...
        else
        {
            if (GS->appTimerDs % 10 == 0) {
                ...
                logjson("DEBUGMOD", "Sent %u: %u, %u, %u, %u\n",
                    GS->appTimerDs,
            ...
            if (floodMessagesPer10Sec != 0)
            {
                ...
                if (numPacketsToSend > 0) {
                    //logjson("DEBUGMOD", "Queuing %u flood packets at time %u\n", numPacketsToSend, GS->appTimerDs);//logt
                    trace( "Queuing %u flood packets at time %02dh %02dm %02ds %03d number" EOL, numPacketsToSend, hour,min,sec,ms);//logt
                }
    ...
    if (GS->appTimerDs >= floodEndTimeDs && floodMode != FloodMode::OFF) {
        logjson("DEBUGMOD", "flood mode timeout\n");//logt
        floodMode = FloodMode::OFF;
    }
#endif
}
...
TerminalCommandHandlerReturnType DebugModule::TerminalCommandHandler(const char* commandArgs[], u8 commandArgsSize)
{
    ...
    if(commandArgsSize >= 3 && (TERMARGS(2 ,moduleName) || TERMARGS(2 ,"eink")))
    {
        ...
        if(commandArgsSize >= 4 && TERMARGS(0 ,"action"))
        {
            ...
            {
                logjson("DEBUGMOD", "send hardfault\n");//logt
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    destinationNode,
            ...
            else if (TERMARGS(3, "counter") && commandArgsSize >= 6)
            {
                ...
                data.packetsPer10Sec = Utility::StringToU16(commandArgs[5]); // orginal Utility::StringToU16(commandArgs[5]);
                data.maxCount = Utility::StringToU32(commandArgs[6]);
                //sizepacket = Utility::StringToU32(commandArgs[7]);
                //trace("%s" EOL ,commandArgs[5]);
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    destinationNode,
                    ...
                    0,
                    (u8*)&data,
                    sizepacket,//  sizepacket SIZEOF_DEBUG_MODULE_START_COUNTER_MESSAGE
                    false // orginal false
                );
                trace("%u",sizepacket);
                return TerminalCommandHandlerReturnType::SUCCESS;
    ...
    {
        logjson("DEBUGMOD", "Flooding has %u packetsIn and %u packetsOut", packetsIn, packetsOut);//logjson
    ...
    {
        u8 checkvar = 1;
        logjson("NODE", "{\"stack\":%u}\n" SEP, (u32)(&checkvar - 0x20000000));
        logjson("NODE", "Module usage: %u\n" SEP, GS->moduleAllocator.GetMemorySize());//logt
    ...
    else if (TERMARGS(0, "advadd") && commandArgsSize >= 4)
    {
        ...
        AdvJob job = {
            ...
            slots,
            delay,
            MSEC_TO_UNITS(32, CONFIG_UNIT_0_625_MS),// orginal 100 
            0, //AdvChannel
            0,
            0,
    ...
    else if (TERMARGS(0, "feed"))
    {
        ...
        logjson("WATCHDOG", "Watchdogs fed.\n");//logt
    ...
    {
        logjson("DEBUGMOD", "cm pending %u\n", GS->cm.GetPendingPackets());//logt
        ...
        for (u32 i = 0; i < conns.count; i++) {
            ...
            logjson("DEBUGMOD", "conn %u pend %u\n", conn->connectionId, conn->GetPendingPackets());//logt
        }
    ...
    if (TERMARGS(0, "p")){
        ...
        sizepacket=Utility::StringToU16(commandArgs[1]); //這裡可以修改封包大小
        logjson("DEBUGMOD","setpayload%u \n",sizepacket);
    }

    if (TERMARGS(0, "clear_array")){
        for(u32 i = 0;i < 7; i ++){
            idtimedelay[i]=0;
            idcounter[i]=0;
            logjson("DEBUGMOD","array claer \n"); //這裡可以把目前陣列的結果先刪除掉
        }
        //logjson("DEBUGMOD","array claer\n");
    }
#endif
...
void printlog(u32 id,u32 time,u32 time2,u32 value,u32 packetmunber ,u32 totaldelay){ //這裡是顯示封包接收資訊的地方
    u32 ms = time%1000; //這裡是顯示時間的毫秒
    u32 tempsec = time/1000;
    u32 sec = tempsec%60; //這裡是顯示時間的秒
    u32 tempmin = tempsec/60;
    u32 min = tempmin%60; //這裡是顯示時間的分
    u32 temphour = tempmin/60;
    u32 hour = temphour%24; //這裡是顯示時間的時
    u32 pdr = packetmunber*100/301; //這裡是封包接收率
    u32 avgdelay = totaldelay/packetmunber; //這裡是平均接收delay
    logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, id, value,hour,min,sec,ms,time-time2,packetmunber,totaldelay,pdr,avgdelay); // find SEP      
}
    ...
    if (packetHeader->messageType == MessageType::MODULE_TRIGGER_ACTION) {
        ...
        if (packet->moduleId == moduleId) {
            ...
            if (actionType == DebugModuleTriggerActionMessages::COUNTER)
            {
                ...
                u32 time2 = data->time; //這裡寫入資料逕行傳送的時間點
                //char timestring[100];// orginal 80 // add
                //GS->timeManager.convertTimestampToString(timestring); // add
                //u32 time3 = GS->timePoint.GetAdditionalTicks();
                u32 time = FruityHal::GetRtcMs();
                u32 ms = time%1000;
                u32 tempsec = time/1000;
                u32 sec = tempsec%60;
                ...
                if(packet->header.sender==241){
                    idcounter[0]=idcounter[0]+1;
                    idtimedelay[0]=idtimedelay[0]+(time-time2);
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[0] ,idtimedelay[0]);
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[0],idtimedelay[0],idcounter[0]*100/151,idtimedelay[0]/idcounter[0]); // find SEP     
                    }

                else if(packet->header.sender==285){
                    idcounter[1]=idcounter[1]+1;
                    idtimedelay[1]=idtimedelay[1]+(time-time2);
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[1] ,idtimedelay[1]);
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[1],idtimedelay[1],idcounter[1]*100/151,idtimedelay[1]/idcounter[1]); // find SEP     
                    }

                else if(packet->header.sender==298){
                    idcounter[2]=idcounter[2]+1;
                    idtimedelay[2]=idtimedelay[2]+(time-time2);   
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[2] ,idtimedelay[2]);
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[2],idtimedelay[2],idcounter[2]*100/151,idtimedelay[2]/idcounter[2]); // find SEP     
                    }

                else if(packet->header.sender==471){
                    idcounter[3]=idcounter[3]+1;
                    idtimedelay[3]=idtimedelay[3]+(time-time2);  
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[3] ,idtimedelay[3]);
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[3],idtimedelay[3],idcounter[3]*100/151,idtimedelay[3]/idcounter[3]); // find SEP     
                    }

                else if(packet->header.sender==1240){
                    idcounter[4]=idcounter[4]+1;
                    idtimedelay[4]=idtimedelay[4]+(time-time2);  
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[4] ,idtimedelay[4]);
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[4],idtimedelay[4],idcounter[4]*100/151,idtimedelay[4]/idcounter[4]); // find SEP     
                    }

                else if(packet->header.sender==1334){
                    idcounter[5]=idcounter[5]+1;
                    idtimedelay[5]=idtimedelay[5]+(time-time2);  
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[5] ,idtimedelay[5]); 
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[5],idtimedelay[5],idcounter[5]*100/151,idtimedelay[5]/idcounter[5]); // find SEP              
                }

                else if(packet->header.sender==1147){
                    idcounter[6]=idcounter[6]+1;
                    idtimedelay[6]=idtimedelay[6]+(time-time2);  
                    printlog(packet->header.sender,time,time2,data->counter,idcounter[6] ,idtimedelay[6]); 
                    //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d delay is %dms received packet number is %d total dealy is %dms PDR is %u avgdelay is %u" SEP, packet->header.sender, data->counter,hour,min,sec,ms,time-time2,idcounter[5],idtimedelay[5],idcounter[5]*100/151,idtimedelay[5]/idcounter[5]); // find SEP              
                }

                //logjson("DEBUGMOD", "sink is %u source is %u" SEP, time, time2); 
                /*u32 delay = 0;
                u32 number = 0;

                for(u32 i = 0;i < 6; i ++){
                    delay = delay + idtimedelay[i];
                    number = number + idcounter[i];
                } 

                u32 delayavg = delay/number;
                u32 pdravg = number*100/1806;*/

                //logjson("DEBUGMOD", "totaldelay is %f totalpdr is %f" SEP, delayavg, pdravg); 

                //logjson("DEBUGMOD", "{\"type\":\"counter\",\"nodeId\":%u,\"value\":%u},Time is currently %02dh %02dm %02ds %03d " SEP, packet->header.sender, data->counter,hour,min,sec,ms); 
                
                if (data->counter  == 0) {
                    counterCheck = 0;
                    logjson("DEBUGMOD", "Resetting counter\n");//logt
                }

                else if (data->counter == counterCheck) {
                    logt("DEBUGMOD", "Counter correct at %u", counterCheck);//logt
                }
                else {
                    logt("DEBUGMOD", "Got wrong counter value %u instead of %u", data->counter, counterCheck);//logt

                }
                counterCheck++;
                //logjson("DEBUGMOD", "Got actually %u should be %u\n", currentCounter, counterCheck);//logt
            }
            else if (actionType == DebugModuleTriggerActionMessages::FLOOD_MESSAGE)
            {
                ...
                if (floodMode == FloodMode::LISTEN) {
                    if (packet->header.sender == floodDestinationId) {
                        ...
                        if (packetsIn != data->packetsOut) {
                            logjson("DEBUGMOD", "Lost messages, got %u should have %u\n", packetsIn, data->packetsOut); //logt
                        }
                    }
                }
                //Listens to all nodes
                    ...
                    if(firstFloodPacketMs && FruityHal::GetRtcMs() < lastFloodPacketMs + 2000){ //orginal 2000
            ...
            {
                logjson("DEBUGMOD", "Resetting flood counter.\n");//logt
                packetsOut = 0;
                packetsIn = 0;
            }
#endif
            ...
            else if (actionType == DebugModuleTriggerActionMessages::GET_JOIN_ME_BUFFER)
            {
                ...
                logjson("DEBUGMOD", "Resetting connection loss counter\n");// logt
                ...
                logjson("DEBUGMOD", "receive hardfault\n");//logt
                CauseHardfault();
            }
        ...
        if (sendData->dataLength >= SIZEOF_CONN_PACKET_HEADER + 3) //We do not need the full data packet, just the bytes that we read
        {
            ...
            logjson("DATA", "IN <= %d ################## Got Data packet %d:%d:%d (len:%d,%s) ##################\n", partnerId, packet->payload.data[0], packet->payload.data[1], packet->payload.data[2], sendData->dataLength.GetRaw(), sendData->deliveryOption == DeliveryOption::WRITE_REQ ? "r" : "u");//logt
        ...
        if(packet->moduleId == moduleId){
            ...
            if (actionType == DebugModuleActionResponseMessages::STATS_MESSAGE){
                ...
                if (sendData->dataLength != MAX_MESH_PACKET_SIZE) {
                    logjson("ERROR", "Packet has an invalid size\n");//logt
                }
...
void DebugModule::CauseStackOverflow() const
{
    ...
    logjson("MAIN", "Dummy data addr: %u\n", (u32)&someDummyData);//logt
    CauseStackOverflow();
}
#ifdef __clang__
