...
void Node::ConfigurationLoadedHandler(u8* migratableConfig, u16 migratableConfigLength)
{
    ...
    if(GET_DEVICE_TYPE() != DeviceType::ASSET && configuration.networkId != 0){
        ...
        AdvJob job = {
            ...
            5, //Slots
            0, //Delay
            MSEC_TO_UNITS(32, CONFIG_UNIT_0_625_MS), //AdvInterval orginal 100 這裡是修改advertising time的地方
            0, //AdvChannel
            0, //CurrentSlots
            0, //CurrentDelay
...
void Node::StartFastJoinMeAdvertising()
{
    ...
    AdvJob job = {
        ...
        0, //Delay
        MSEC_TO_UNITS(32, CONFIG_UNIT_0_625_MS), //AdvInterval
        0, //AdvChannel
        0, //CurrentSlots
        0, //CurrentDelay
        FruityHal::BleGapAdvType::ADV_IND, //Advertising Mode
    ...
    if (GS->cm.freeMeshOutConnections > 0)
    {
        ...
        if (bestClusterAsMaster != nullptr)
        {
            ...
            /*if(bestClusterAsMaster->payload.deviceType == DeviceType::LEAF){
                connectionIv = MSEC_TO_UNITS(200, CONFIG_UNIT_1_25_MS); // orginal 90
            }*/
...
{
    //這裡是作為master節點選擇其他節點的篩選條件，如果有達成以下條件就不考慮該節點做為連線對象
    //If the packet is too old, filter it out  // 2
    if (GS->appTimerDs - packet.receivedTimeDs > MAX_JOIN_ME_PACKET_AGE_DS) return 0;

    //If we are already connected to that cluster, the score is 0  // 3
    if (packet.payload.clusterId == this->clusterId) return 0;

    //If there are zero free in connections, we cannot connect as master // 4
    if (packet.payload.freeMeshInConnections == 0) return 0;

    //If the other node wants to connect as a slave to another cluster, do not connect // 1
    if (packet.payload.ackField != 0 && packet.payload.ackField != this->clusterId) return 0;
    //if (packet.payload.ackField != this->clusterId) return 0;// add

    //If the other cluster is bigger, we cannot connect as master  // 5
    ...
    //Do not connect if we are already connected to that partner // 6
    if (GS->cm.GetMeshConnectionToPartner(packet.payload.sender)) return 0;

    //Connection should have a minimum of stability // 7
    ...
    //這裡是作為master節點評分其他符合條件的節點並評分的地方
	...
	if(GET_DEVICE_TYPE() == DeviceType::SINK)
		isSink = 1;
	else if(configuration.nodeId == 471 || configuration.nodeId == 1334)
		isPrio = 1;
    //add Qos
    u32 score = 0;

    if(isPrio == 1){
        score = isSink*10000 - (u32)(packet.payload.hopsToSink) * 1000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
        //score = isSink*10000 - (u32)(packet.payload.hopsToSink) * 2000 + (u32)(packet.payload.freeMeshOutConnections) * 500 + rssiScore;
    }

    else{
        score = isPrio*10000 - (u32)(packet.payload.hopsToSink) * 1000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
        //score = isSink*10000 - (u32)(packet.payload.hopsToSink) * 2000 + (u32)(packet.payload.freeMeshOutConnections) * 500 + rssiScore;
    }
    ...
    /*u32 isPrio = 0;
	if(configuration.nodeId == 471 || configuration.nodeId == 1334)
		isPrio = 1;

    u32 score = 0;
    if(isPrio == 1){
        score = (u32)(packet.payload.freeMeshInConnections) * 10000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
    }

    else{
        score = (u32)(packet.payload.freeMeshInConnections) * 5000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
    } //add*/
    //u32 score = (u32)(packet.payload.freeMeshInConnections) * 10000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore; //orginal
    /*
    u32 score = 0;
    if(GET_DEVICE_TYPE() == DeviceType::SINK){
        score = 100;
...
{
    //這裡是作為slave節點選擇其他節點的篩選條件，如果有達成以下條件就不考慮該節點做為連線對象
    //If the packet is too old, filter it out // 1
    if (GS->appTimerDs - packet.receivedTimeDs > MAX_JOIN_ME_PACKET_AGE_DS) return 0;

    //If we are already connected to that cluster, the score is 0 // 2
    if (packet.payload.clusterId == this->clusterId) return 0;

    //Do not check for freeOut == 0 as the partner will probably free up a conneciton for us and we should be ready

    //We will only be a slave of a bigger or equal cluster //3
    if (packet.payload.clusterSize < GetClusterSize()) return 0;

    //Connection should have a minimum of stability // 4
    if(packet.rssi < STABLE_CONNECTION_RSSI_THRESHOLD) return 0;
    ...
    u32 isSink = 0;
	u32 isPrio = 0;
	if(GET_DEVICE_TYPE() == DeviceType::SINK)//這裡是確認自己是否為Sink節點
		isSink = 1;
	else if(configuration.nodeId == 471 || configuration.nodeId == 1334)
		isPrio = 1;
    u32 score = 0;
    ...
    if(isPrio == 1){
        score = isSink*10000 - (u32)(packet.payload.hopsToSink) * 1000 + (u32)(packet.payload.clusterSize) * 100 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
        //score = isSink*10000 - (u32)(packet.payload.hopsToSink) * 2000 + (u32)(packet.payload.clusterSize) * 100 + (u32)(packet.payload.freeMeshOutConnections) * 500 + rssiScore;
    }
        ...
        //score = (u32)(packet.payload.hopsToSink) * 2000 + (u32)(packet.payload.clusterSize) * 100 + (u32)(packet.payload.freeMeshOutConnections) * 500 + rssiScore;
    }//add
    ...
    if(isPrio == 1){
        ...
        u32 score = (u32)(packet.payload.clusterSize) * 5000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore;
    } //add */

    //u32 score = (u32)(packet.payload.clusterSize) * 10000 + (u32)(packet.payload.freeMeshOutConnections) * 100 + rssiScore; //orginal
...
//    if(numGoodNodesInBuffer >= Config->numNodesForDecision) ...
    ...
    if(rebootTimeDs != 0 && rebootTimeDs < GS->appTimerDs){
        ...
        *GS->rebootMagicNumberPtr = REBOOT_MAGIC_NUMBER;
        ...
        FruityHal::DelayMs(500);//orginal 500
...
TerminalCommandHandlerReturnType Node::TerminalCommandHandler(const char* commandArgs[], u8 commandArgsSize)
{
    ...
    {
        char timestring[100];// orginal 80
    ...
    else if(TERMARGS(0, "update_iv"))     //jstodo can this be removed? Currently untested
    {
        ...
        switch (newConnectionInterval)
        {
            ...
            case MSEC_TO_UNITS(200, CONFIG_UNIT_1_25_MS):
                break;
            default:
        ...
        logjson("DEBUGMOD", "Now connection interval is %u", newConnectionInterval);//logt
